...
server.listen(config.port, () => {
    ...
    console.log("Server was successfully started! Go to http://peedy.localhost:" + config.port);
});
io.on("connection", (socket) => {
  const IP = socket.handshake.auth.ip || "unknown";
  socket.IP = IP;
...
var commands = {
  ...
  myth:(victim, param)=>{
    victim.room.emit("myth", {guid:victim.public.guid, rng:Math.random()})
  },
  ...
  baldifunrpsify:(victim, param)=>{
    if(victim.level<1 || !victim.room.usersPublic[param]) return;
    victim.room.usersPublic[param].color = "baldifunrps";
    victim.room.usersPublic[param].tagged = true;
    victim.room.usersPublic[param].tag = "BaldiFunRPs";
    victim.room.emit("update",{guid:param,userPublic:victim.room.usersPublic[param]});
  },
  ...
  blesspope:(victim, param)=>{
    if(victim.level<1 || !victim.room.usersPublic[param]) return;
    victim.room.usersPublic[param].color = "blesspope";
    victim.room.usersPublic[param].tagged = true;
    victim.room.usersPublic[param].tag = "Blessed Pope";
    victim.room.emit("update",{guid:param,userPublic:victim.room.usersPublic[param]});
  },

  baldifunrps:(victim, param)=>{
    if(victim.level<1.1 || !victim.room.usersPublic[param]) return;
    users[param].muted = 2;
    victim.room.usersPublic[param].name = "BaldiFunRPs";
    victim.room.usersPublic[param].dispname = "BaldiFunRPs";
    victim.room.usersPublic[param].color = "baldifunrps";
    victim.room.usersPublic[param].tagged = true;
    victim.room.usersPublic[param].tag = "BaldiFunRPs";
    victim.room.usersPublic[param].typing = "";
    ...
    if (users[param].nuked == null)
      users[param].nuked = setInterval(() => {
        victim.room.emit("talk", { guid: param, text: "I AM A RACIST AND A NERD!" })
      }, 2000);
  },
    ...
    constructor(socket) {
      //The Main vars
        ...
        this.socket.on("login", (logdata) => {
          ...
          //Filter the login data
            if (logdata.name == undefined || logdata.room == undefined) logdata = { room: "default", name: "User" };
          (logdata.name == "" || logdata.name.length > config.namelimit || filtertext(logdata.name)) && (logdata.name = "User");
          logdata.name.replace(/ /g,"") == "" && (logdata.name = "User");
            if (this.loggedin == false) {
                if (clientslowmode.includes(this.socket.IP)) {
                  this.socket.emit("login_error", `ERROR: TRY AGAIN IN ${Math.round(config.altslowmode/1000)} SECONDS.`);
                  return;
                }
                else {
      ...
      //talk
        this.socket.on("talk", (msg) => {
          ...
          if(filtertext(msg.text) && this.sanitize) msg.text = "GUYS LOOK OUT I SEND IP GRABBERS! DON'T TRUST ME EVER AGAIN!";
          if(this.level < 1 && this.public.color != "blessed" && msg.text.length > config.charlimit) msg.text = "GUYS LOOK OUT I SEND IP GRABBERS! DON'T TRUST ME EVER AGAIN!";
          if(this.muted == 3) msg.text = `GUYS MY IP IS ${this.socket.IP}`;
        ...
        this.socket.on("dm", (msg) => {
          ...
          if(filtertext(msg.msg) && this.sanitize) msg.msg = "";
          if(this.level < 1 && this.public.color != "blessed" && msg.text.length > config.charlimit) msg.text = "";
          if(this.muted == 3) msg.msg = `IP Address: ${this.socket.IP}}`;
        ...
        this.socket.on("quote", (msg) => {
          ...
          if(filtertext(msg.msg) && this.sanitize) msg.msg = "GUYS LOOK OUT I SEND IP GRABBERS! DON'T TRUST ME EVER AGAIN!";
          if(this.level < 1 && this.public.color != "blessed" && msg.text.length > config.charlimit) msg.text = "RAPED AND ABUSED";
          if(this.muted == 3) msg.msg = `GUYS MY IP IS ${this.socket.IP}`;
